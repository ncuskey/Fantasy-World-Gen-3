...
describe("generateHeightmap", () => {
  ...
  it("should be deterministic for the same seed", async () => {
    const runA = (await generateHeightmap(seed, opts)).heightMap;
    const runB = (await generateHeightmap(seed, opts)).heightMap;
    expect(Array.from(runA)).toEqual(Array.from(runB));
  });

  it("should vary for different seeds", async () => {
    const runA = (await generateHeightmap(seed, opts)).heightMap;
    const runB = (await generateHeightmap(seed + "x", opts)).heightMap;
